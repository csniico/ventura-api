...
import { emitUserEvent, UserEvents } from '../events/user.events'
...
export class UserServiceV2 {
  ...
  async verifyCredentials(
    ...
    if (!user || !user.password) {
      await argon2.verify(UserServiceV2.DECOY_HASH, password).catch(() => false)
      return null
    }
  ...
  async findByAppleId(appleId: string): Promise<IUser | null> {
    return await this.userRepository.findByAppleId(appleId)
  }
